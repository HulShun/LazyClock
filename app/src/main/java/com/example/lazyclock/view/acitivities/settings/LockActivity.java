...
public class LockActivity extends Activity {
    ...
    private void initData() {
        ...
        //关闭锁定的时候用到
        if (intent.getFlags() == Config.CHECK_PASSWORDS) {
            state = Config.CHECK_PASSWORDS;
        }
        // 初次设定密码
        ...
        mLockEditText4.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                //输入新密码完,或者关闭锁定时验证密码后，关闭输入法
                    ...
                    hideInputMethod();
                } else if (state == Config.CHECK_PASSWORDS) {
                    if (checkPasswords()) {
                        setResult(Config.CHECK_PASSWORDS);
                ...
                if (state == CHANGEPASSWORDS || state == CHECK) {
                    ...
                    if (sb.toString().length() < 4) {
                        ...
                        try {
                            ...
                            FileUtil.getInstence().saveSetting(myApp);
                            LockActivity.this.finish();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (hideInputMethod()) {
                if (state == Config.CHECK_PASSWORDS) {
                    LockActivity.this.finish();
                } else {
                    LockActivity.this.finish();
                }
